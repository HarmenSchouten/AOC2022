...
for (let i = 0; i < 10; i++) {
    itemsCopy = round(itemsCopy, elves)
}

const minX = Math.min(...elves.map(elve => elve.x))
const maxX = Math.max(...elves.map(elve => elve.x))
const minY = Math.min(...elves.map(elve => elve.y))
const maxY = Math.max(...elves.map(elve => elve.y))

const numberOfEmptyFields = itemsCopy.reduce((acc, line, y) => {
    if (y < minY || y > maxY) {
        return acc
    }
    return acc + line.reduce((acc, field, x) => {
        if (x < minX || x > maxX) {
            return acc
        }
        return acc + (field === "." ? 1 : 0)
    }, 0)
}, 0)
