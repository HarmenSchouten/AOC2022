...
items.forEach(item => {
    ...
    // Create Direcotry
        ...
        addFolderToFolder(currentDirKey, {name: name, files: [], key: window.crypto.randomUUID()} as Folder)
    } 
    // Create file
    else {
        const [size, name] = item.split(" ");
        addFileToFolder(currentDirKey, {name, size: parseInt(size)} as File)
    }
});
...
const root = dirs[dirs.length - 1].dirSize
const maxSize = 70000000
const reqFreeSpace = 30000000 - (maxSize - root)
console.log(reqFreeSpace)
