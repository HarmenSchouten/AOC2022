...
for (let i = 0; i < 5000 && cycle.size < 2; i++) {
    const rock = rocks[i % 5];
    const x = startX;
    const y = chamber.length + margin
    chamber = dropRock(chamber, rock, x, y)
    cycleMap[i] = chamber.length
    if (chamber[chamber.length - 1].every(cell => cell === "#")) {
        cycle.add(i)
    }
}

const diff = [...cycle][1] - [...cycle][0]
const startCycle = cycleMap[[...cycle][0]]
const blockDiff = cycleMap[[...cycle][1]] - cycleMap[[...cycle][0]]

const division = ((1000000000000 - [...cycle][0]) / diff)
console.log(Math.round(division * blockDiff + startCycle))
